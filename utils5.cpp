...
static inline double galutinis(const Student& st, Mode m) {
    return (m == Mode::Vid ? st.getGalVid() : st.getGalMed());
}
...
std::vector<Student> generuoti_studentus(int m) {
    ...
    for (int i = 1; i <= m; ++i) {
        ...
        std::vector<double> nd(15);
        for (int k = 0; k < 15; ++k) nd[k] = static_cast<double>(d(rng));
        double egz = static_cast<double>(d(rng));
        Student st(pav, var, nd, egz);
...
void irasyti_pilna_faila(const std::string& path, const std::vector<Student>& s) {
    ...
    for (const auto& st : s) {
        out << std::left << std::setw(20) << st.getVardas()
            << std::left << std::setw(20) << st.getPavarde();

        const auto& nd = st.getNd(); 
        for (double ndv : nd)
            out << std::right << std::setw(5) << static_cast<int>(ndv);
        out << std::right << std::setw(5) << static_cast<int>(st.getEgz())
            << "\n";
    }
}
...
void rasyti_grupe(const std::string& path, const std::vector<Student>& s, Mode m) {
    ...
    for (const auto& st : s) {
        out << std::left << std::setw(20) << st.getPavarde()
